...
function [net,info,monitor,net_name] = ISS_train(train_dataset, valid_dataset, ... % Data
    ...
    learn_rate, max_epochs, mini_batch, varargin) % Training
...
%   hidden_units    [N x 1 double]  Number of hidden unit, comprehensive of the initial fully connected layer, implicit number of layers N
...
num_layers = height(hidden_units) - 1;  % Remove the FC layer in RNN count
...
for i = 1:num_layers
    u_max{i} = ones(hidden_units(i),1);
end

network_layers = net_maker(is_lstm, hidden_units, num_layers, num_features, num_responses, dropout_rate);
